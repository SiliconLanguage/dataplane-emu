#!/usr/bin/env python3
"""
Export the Double Trampoline architecture diagram to high-resolution PNG.

Reads the element data from the .excalidraw JSON and renders it with Pillow.
Produces a pixel-accurate, publication-quality image.

Usage:
  python3 docs/arch/export_png.py
"""
...
def get_font(size, bold=False):
    """Load a TrueType font at the given size, with fallback to default."""
...
def hex_to_rgba(hex_color, alpha=255):
    ...
    h = hex_color.lstrip("#")
    if len(h) == 6:
        return (int(h[0:2], 16), int(h[1:4][:2], 16), int(h[4:6], 16), alpha)
    return (0, 0, 0, alpha)
...
def draw_line(draw, el):
    ...
    if len(points) < 2:
        return
    stroke = hex_to_rgb(el.get("strokeColor", "#1e1e1e"))
...
def draw_arrow(draw, el):
    """Draw an arrow element with arrowhead."""
    ox, oy = el["x"], el["y"]
    points = el.get("points", [])
    if len(points) < 2:
        return
...
def _draw_arrowhead(draw, from_pt, to_pt, color, line_width):
    """Draw a filled arrowhead at to_pt pointing away from from_pt."""
...
def draw_text(draw, el, all_els):
    """Draw a text element, resolving container binding."""
    content = el.get("text", "")
    if not content:
        return
    ...
    # Determine if this is bold (layer titles are larger)
    bold = font_size >= 24
...
def main():
    ...
    min_x = min(e["x"] for e in elements if e["type"] in ("rectangle", "text", "arrow", "line"))
    min_y = min(e["y"] for e in elements if e["type"] in ("rectangle", "text", "arrow", "line"))
    ...
    # Sort rects: larger area first (backgrounds first)
...
if __name__ == "__main__":
    if not os.path.exists(EXCALIDRAW_PATH):
        print(f"✗ {EXCALIDRAW_PATH} not found. Run generate_diagram.py first.")
        sys.exit(1)
    success = main()

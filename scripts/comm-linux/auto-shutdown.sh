...
if [ "$IDLE_COUNT" -ge "$MAX_IDLE" ]; then
    ...
    # Execute deallocation and immediately wipe the token
...
# If idle for 60 minutes, tell Azure to deallocate the VM
if [ "$IDLE_COUNT" -ge "$MAX_IDLE" ]; then
    ...
    curl -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Length: 0" "https://management.azure.com/subscriptions/$SUB/resourceGroups/$RG/providers/Microsoft.Compute/virtualMachines/$VM/deallocate?api-version=2021-03-01"
fi
